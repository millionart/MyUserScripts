...
(function () {
    ...
    function scheduleRefresh() {
        ...
        refreshTimer = window.setTimeout(async () => {
            refreshTimer = null;
            injectLanguageSetting();
            await refreshPageLanguage(currentLanguageMode);
        }, 80);
    }
    ...
    function installObservers() {
        ...
        if (!routeWatcher) {
            ...
            routeWatcher = window.setInterval(() => {
                if (window.location.href === routeHref) return;
                routeHref = window.location.href;
                scheduleRefresh();
            }, 500);
        }
    ...
    async function initializeShell() {
        ensureStyles();
        const storedMode = normalizeLanguageMode(await GM_getValue(LANGUAGE_STORAGE_KEY, DEFAULT_LANGUAGE_MODE));
        currentLanguageMode = storedMode;
        const marker = ensureVersionMarker();
        if (marker) {
            marker.setAttribute('data-language-mode', storedMode);
        }
        injectLanguageSetting();
